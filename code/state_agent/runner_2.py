...
class Match:
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
        ...
        for it in range(max_frames):
            ...
            print("time update: ", time.time() - tm_update)

            # Have each team produce actions (in parallel)
            tm_delayed = time.time()   
            if t1_can_act:
                if t1_type == 'image':
            ...
            print("time delayed: ", time.time() - tm_delayed)


            tm_actions = time.time()   
            ...
            print("time actions: ", time.time() - tm_actions)
            ...
            actions_list = [self._pystk.Action(**a) for a in actions]
            tm_step = time.time()
            logging.debug('  race.step  [score = {}]'.format(state.soccer.score))
            if (not race.step(actions_list) and num_player) or sum(state.soccer.score) >= max_score:
                break

            print("time step: ", time.time() - tm_step)
