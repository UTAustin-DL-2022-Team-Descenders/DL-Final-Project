...
class BaseActor:
    ...
    def __init__(self, action_net, train=None, reward_type=None, sample_type=None):
        ...
        # Set model path and name to save/load SteeringActor's action_net
    ...
    def save_model(self, custom_model_name=None):

        # set the name of the model. User may provide a custom model name or default to self.model_name
        save_model_name = custom_model_name if custom_model_name else self.model_name

        save_model(self.action_net, save_model_name, save_path=self.model_path, use_jit=True)

    def load_model(self, custom_model_name=None):

        # set the name of the model. User may provide a custom model name or default to self.model_name
        load_model_name = custom_model_name if custom_model_name else self.model_name

        self.action_net = load_model(load_model_name, load_path=self.model_path, use_jit=True)
