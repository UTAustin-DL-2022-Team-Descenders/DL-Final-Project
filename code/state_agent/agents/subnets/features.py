...
def get_opponent_center(opponent_state):
  return get_object_center(opponent_state)
...
class OpponentFeatures(Features):
    ...
    PLAYER_OPPONENT_DISTANCE = 2
    # PLAYER_WALL_DISTANCE = 3
    # OPPONENT_GOAL_DISTANCE = 5
    # PREVIOUS_SPEED = 29
    # DELTA_SPEED_BEHIND = 30
    # TARGET_SPEED_BEHIND = 31
    SPEED = 32
    ...
    # PREVIOUS_ACCEL = 35
    # PLAYER_GOAL_ANGLE = 39
    # PLAYER_OPPONENT_COUNTER_STEER_ANGLE = 40
    # STEERING_ANGLE_BEHIND = 41
    PLAYER_OPPONENT_ANGLE = 42
    # STEERING_ANGLE = 43
    # PLAYER_OPPONENT_GOAL_ANGLE = 44

    def get_feature_vector(self, kart_info, opponent_state, absolute=False, target_speed=0.0, last_state=None, last_action=None, **kwargs):
        # cart location
        p = cart_location(kart_info)
        ...
        # opponent
        opponent = get_opponent_center(opponent_state)
        ...
        steer_angle = get_obj1_to_obj2_angle(p, front)
        # steer_angle_behind = get_obj1_to_obj2_angle(p, front) + np.pi
        steer_angle_opponent = get_obj1_to_obj2_angle(front, opponent)
        # steer_angle_goal = get_obj1_to_obj2_angle(p, goal)
        # steer_angle_opponent_goal = get_obj1_to_obj2_angle(opponent, goal)
        # steer_angle_goal_diff = get_obj1_to_obj2_angle_difference(steer_angle, steer_angle_goal)
        steer_opponent_angle_diff = get_obj1_to_obj2_angle_difference(steer_angle, steer_angle_opponent)
        # #steer_opponent_goal_angle_diff = steer_angle_goal_diff - steer_opponent_angle_diff

        # # counter steering up to 30 degrees (1/6 pi)
        # #steer_angle_opponent_goal_counter_steer = -1 * np.sign(steer_opponent_angle_diff) * (np.clip(steer_opponent_goal_angle_diff, -0.15, 0.15)) + steer_opponent_angle_diff
        # steer_angle_opponent_goal_counter_steer = limit_period(steer_opponent_angle_diff - np.clip(steer_angle_goal_diff, -OPPONENT_MAX_STEER_OFFSET, OPPONENT_MAX_STEER_OFFSET))
        # #get_obj1_to_obj2_angle_difference(steer_angle, steer_angle_opponent)  #steer_opponent_angle_diff #np.sign(steer_opponent_angle_diff) * np.clip(np.abs(steer_opponent_angle_diff), 0, 1.0/100.0)

        # distance
        pp_dist = np.linalg.norm(p - opponent) - PUCK_RADIUS
        ...
        # features[0:2] = p - opponent
        # features[self.PLAYER_WALL_DISTANCE] = (np.linalg.norm(p) - NEAR_WALL_OFFSET) / (NEAR_WALL_STD)
        features[self.PLAYER_OPPONENT_DISTANCE] = pp_dist
        # features[self.OPPONENT_GOAL_DISTANCE] = np.linalg.norm(opponent - goal)
        features[self.SPEED] = speed
        # features[self.PREVIOUS_ACCEL] = last_action.acceleration if last_action is not None else 0.0
        # features[self.PREVIOUS_SPEED] = previous_speed
        features[self.TARGET_SPEED] = target_speed
        ...
        # features[self.STEERING_ANGLE_BEHIND] = steer_angle_behind
        # features[self.PLAYER_GOAL_ANGLE] = steer_angle_goal_diff
        features[self.PLAYER_OPPONENT_ANGLE] = steer_opponent_angle_diff
        # features[self.PLAYER_OPPONENT_GOAL_ANGLE] = 0  # steer_opponent_goal_angle_diff
        # features[self.PLAYER_OPPONENT_COUNTER_STEER_ANGLE] = steer_angle_opponent_goal_counter_steer
    ...
    #
    # def select_player_puck_goal_angle(self, features):
    #     return features[self.PLAYER_OPPONENT_GOAL_ANGLE]
    #
    # def select_player_goal_angle(self, features):
    #     return features[self.PLAYER_GOAL_ANGLE]

    def select_player_puck_angle(self, features):
        return features[self.PLAYER_OPPONENT_ANGLE]

    # def select_behind_player_angle(self, features):
    #     return features[self.STEERING_ANGLE_BEHIND]
    #
    # def select_player_puck_countersteer_angle(self, features):
    #     return features[self.PLAYER_OPPONENT_COUNTER_STEER_ANGLE]
    #
    # def select_lateral_distance(self, features):
    #     return 0
    ...
    def select_player_puck_distance(self, features):
        return features[self.PLAYER_OPPONENT_DISTANCE]

    # def select_puck_goal_distance(self, features):
    #     return features[self.OPPONENT_GOAL_DISTANCE]
